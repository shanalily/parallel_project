...
unsigned int mk_cars(float proportion){ 
    ...
    for (int i = 0; i < CAR*miles_per_rank; i += CAR) {
        ...
        if (interstate_west[i]) {
            interstate_west[i+1] = (int) (GenVal(row)*SIZE);
            total_cars += 1;
        }
        ...
        if (interstate_east[i]) {
            interstate_east[i+1] = (int) (GenVal(row+1)*SIZE);
            total_cars += 1;
        }
...
void update_roads(unsigned int miles_per_rank, unsigned int *interstate_west, unsigned int *interstate_east,
        int can_move_west, int can_move_east) {
...
void run_simulation(unsigned int *interstate_west, unsigned int *interstate_east) {
    int c1[1], c2[1], m1[1], m2[2];
    int total_reached = 0, empty_val = -1, can_move_west=0, can_move_east=0;
    int *empty = &empty_val;
    for (int i = 0; i < NUM_TICKS; ++i) {
        ...
        // update roads
        update_roads(miles_per_rank, interstate_west, interstate_east, can_move_west, can_move_east);
...
int main(int argc, char *argv[]) {
    ...
    printf("total cars: %d\n", total_cars);
    // print_interstate();
    run_simulation(interstate_west, interstate_east);
